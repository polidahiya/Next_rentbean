...
export const generateMetadata = ({ params }) => {
  ...
  return {
    title: product?.name + " on rent in " + " | Rentbean.in",
  };
};
  ...
  return (
    <div>
      <div className="flex items-center gap-[5px] h-[30px] px-[10px] lg:px-[40px] bg-bg1 text-[12px] text-theme select-none ">
        ...
        &gt;
        <Link className="hover:text-cyan-500" href={"/" + params.category}>
          {category}
        </Link>{" "}
        &gt;
        <Link
        ...
        &gt;
        <div className="whitespace-nowrap overflow-hidden text-ellipsis max-w-[100%]">{product.name}</div>
      </div>
